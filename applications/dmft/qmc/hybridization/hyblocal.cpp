...
//    std::cerr << " done\n";
  ...
  // We comment out the below to 1) make my changes go by more smoothly, and 2) make it so that the user has to apply the static part of the changes (not necessary, but easiest for now)
...
//this is an O(k n_orbital) procedure in the expansion order and the number of orbitals.
double local_configuration::local_weight_change(const segment &seg, int orb, bool antisegment) const{
  ...
  if(use_retarded_interaction_){
    ...
    if((seg.t_start_==0) && (seg.t_end_==beta_)){ //not really a segment but a full line
      ...
      for(int i=0;i<n_orbitals_;++i){
        for(std::set<segment>::const_iterator it=segments_[i].begin(); it != segments_[i].end();++it){
          retarded_weight+=sgn*K_.interpolate(seg.t_start_-it->t_start_,orb, i);
          retarded_weight-=sgn*K_.interpolate(seg.t_start_-it->t_end_,orb, i);
          retarded_weight-=sgn*K_.interpolate(seg.t_end_-it->t_start_,orb,i);
          retarded_weight+=sgn*K_.interpolate(seg.t_end_-it->t_end_,orb,i);
      ...
      if(is_removal){
        retarded_weight+=K_.interpolate(seg.t_end_-seg.t_start_,orb,orb);
      }else{
        retarded_weight-=K_.interpolate(seg.t_end_-seg.t_start_,orb,orb);
      }
    }
...
double local_configuration::local_energy(const segment &seg, int orb,bool d_mu_only) const{
    ...
    if(use_retarded_interaction_){
        ...
        if((seg.t_start_==0) && (seg.t_end_==beta_)){ //not really a segment but a full line
            ...
            for(int i=0;i<n_orbitals_;++i){
                for(std::set<segment>::const_iterator it=segments_[i].begin(); it != segments_[i].end();++it){
		  retarded_weight+=K_.interpolate(seg.t_start_-it->t_start_,orb,i);
		  retarded_weight-=K_.interpolate(seg.t_start_-it->t_end_,orb,i);
		  retarded_weight-=K_.interpolate(seg.t_end_-it->t_start_,orb,i);
		  retarded_weight+=K_.interpolate(seg.t_end_-it->t_end_,orb,i);
                }
            }
            retarded_weight-=K_.interpolate(seg.t_end_-seg.t_start_,orb,orb);
        }
...
double local_configuration::interaction_density_integral(std::set<segment>::const_iterator &it_i) const{
  ...
  for(int j=0; j<n_orbitals_; ++j){
    for(std::set<segment>::const_iterator it_j=segments_[j].begin(); it_j!=segments_[j].end();++it_j){
      integral+= K_.interpolate_deriv(it_j->t_end_ - it_i->t_end_,j) - K_.interpolate_deriv(it_j->t_start_ - it_i->t_end_,j);
      remove+=2*K_.interpolate_deriv(0.0,j)/n_orbitals();
    }
  }
  return integral-remove; //this is the same segment, same time contribution due to the fact that n and c do not commute in this case
}
