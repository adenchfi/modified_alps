...
ret_int_fun::ret_int_fun(const alps::params &p):
  green_function<double>(p["N_TAU"]+1, p["N_ORBITALS"].cast<int>(), 2)//two "flavors" K, K' (K' used for improved estimator), but "sites" (orbitals) m, n
{
...
for(std::size_t i=0; i<ntime();++i)
  for(std::size_t j=0; j<nflavor();++j)
    for(std::size_t k=0; k<nsite();++k)
      for(std::size_t kp=0; kp<nsite();++kp)
	if(operator()(i,k,kp,j) < 0. && j==0) {
	  std::cout << "Problem. K = " << operator()(i,k,kp,j) << "at i = " << i << " and k = " << k << " and kp = " << kp << std::endl;
	  throw std::invalid_argument("Problem with retarded interaction function: RET_INT_K(\\tau) < 0. K should always be positive! But it's not!");
	}
...
void ret_int_fun::read_interaction_K_function(const alps::params &p){
  ...
  if(p.defined("K_IN_HDF5") && p["K_IN_HDF5"].cast<bool>()){//attempt to read from h5 archive
    ...
    for (std::size_t i=0; i<ntime(); i++) {
      ...
      for (std::size_t j=0; j<nflavor(); j++){
	for (std::size_t k=0; k<p["N_ORBITALS"].cast<int>(); k++){
	  for (std::size_t kp=0; kp<p["N_ORBITALS"].cast<int>(); kp++){
	    if(!infile.good()){
	      std::cout << "Got to j = " << j << " and k = " << k << "and kp = " << kp << std::endl;
	    ...
	    double delta;
	    infile >> delta;
	    operator()(i,k,kp,j)=delta;
	    if (i == 1){
	      std::cout << "Reading value for i=1 " << delta << " at j = " << j << " and k = " << k << "and kp = " << kp << std::endl;
	    }
	  }
	}
      }
...
double ret_int_fun::interpolate(double time, int orb1, int orb2) const{
  time=std::abs(time);

  double n = time/beta_*(ntime()-1);
  int n_lower = (int)n; // interpolate linearly between n_lower and n_lower+1

  return operator()(n_lower,orb1,orb2,0) + (n-n_lower)*(operator()(n_lower+1,orb1,orb2,0)-operator()(n_lower,orb1,orb2,0));
}

//linear interpolation of the retarded interaction function.
double ret_int_fun::interpolate_deriv(double time, int orb1) const{
  //if(time<-beta_ || time > beta_) std::cout<< "!!" << std::endl;
  if(time<0.) return -1.*interpolate_deriv(-time,orb1);

  double n = time/beta_*(ntime()-1);
  int n_lower = (int)n; // interpolate linearly between n_lower and n_lower+1
  //double dtau = beta_/(ntime());
  //kplower = (operator()(n_lower+1, orb1, orb1, 0) - operator()(n_lower, orb1, orb1, 0))/dtau;
  return operator()(n_lower,orb1,orb1,1) + (n-n_lower)*(operator()(n_lower+1,orb1,orb1,1)-operator()(n_lower,orb1,orb1,1));
}
